...
class TasksCog(commands.Cog):
    ...
    async def check_task_updates(self):
        ...
        for user_id, info in data.items():
            for task_type in ["daily", "weekly", "monthly"]:
                ...
                channel_name = f"{task_type}-tasks"
                channel = None
    ...
    async def process_task_message(self, message, is_edit=False):
        ...
        if task_type == "daily" and is_after_11_am():
            ...
            save_data(data)
            await message.channel.send(
                f"{message.author.mention} Daily tasks after 11 AM are not accepted. Fine Rs.120 added. Total Fine: Rs.{data[user_id]['fine']}"
            )
            return
        ...
        if task_type == "daily" and total < 2:
            await message.channel.send(
                f"{message.author.mention} Minimum 2 daily tasks required. You set {total} task(s). Fine Rs.120 will apply."
            )

        if data[user_id][task_type].get("date") == today_str():
            if not is_edit:
                pass
        else:
            if not is_edit:
